...
function showValue() {
  ...
  if(playerHand.value > 21){
    $('#gameBoard').append('<div class="player-value">Te pasaste!</div');
    showCompValue();
    computerTurn();
    $('.buttons').addClass('hide');
  }
...
function showCompValue() {
  ...
  if(computerHand.value > 21){
    $('#gameBoard').append('<div class="computer-value">Presley se paso!</div');
  }
}
...
function updateCashBet() {
  $('#cash').text('Tu Dinero: $' + cash.toString());
  $('#bet').text('Apuestas: $' + bet.toString());
}

function win() {
  $('#gameBoard').append('<div class="result">Ganaste $' + bet.toString() + '!</div');
  cash += bet;
  updateCashBet();
  playSound('win');
}
...
function draw() {
  $('#gameBoard').append('<div class="result">Sacaste!</div');
  playSound('draw');
}

function lose() {
  $('#gameBoard').append('<div class="result">Perdiste $' + bet.toString() + '!</div');
  cash -= bet;
  updateCashBet();
  if( cash < bet ) {
    bet = 100;
